...
class GestureProcessor(object):
    def __init__(self, gestureFile="gestureData.txt",is_vid="True"):
        ...
        self.isPpt= not self.isVideo
        print('isppt',self.isPpt,'isVideo',self.isVideo)
        self.saveNextGesture = False
        self.lastAction = ""
        self.handMomentPositions = []
        self.handCenterPositions = []
        self.timed_counter = 0
        self.initGestures()
...
# to make processing easier, and ultimately extracting the contour.
    ...
    def threshold(self):
        ...
        hist,bins = np.histogram(self.original.ravel(),256,[0,256])
        peak = np.argmax(hist)
        value = (31, 31)
        ...
        if(peak > 60) :
            ...
            _, self.thresholded = cv2.threshold(blurred, 127, 255,cv2.THRESH_OTSU)
    ...
    def extractHandContour(self):
        ...
        # create bounding rectangle around the contour in red 
...
# Functions associated with determining gestures
    ...
    def determineIfGesture(self):
        if self.stationary and self.isVideo:
            self.timed_counter +=1
            if self.timed_counter >= 36:   
                if self.count_defects == 2:
                    ...
                    cv2.putText(self.real,"No result", (50, 50),cv2.FONT_HERSHEY_SIMPLEX, 2, 2)
        if self.record:
            ...
            if len(self.gesturePoints) > minGesturePoints:
                ...
                if gestureIndex != None:
                    self.gestures[gestureIndex].action(self.isPpt)
